...
import fs from "fs";
import Mellis from "../models/mellisImg.js"; 
...
router.get("/", async (req, res) => {
    try {
        const imgs = await Mellis.findAll()
        res.json(imgs)
...
router.get("/:id", async (req, res) => {
    ...
    try {
        const image = await Mellis.findByPk(id);
        if (!image) {
...
router.post("/", fileUpload, async (req, res) => {
    try {
        ...
        const newImage = await Mellis.create({
            name: originalname,
            type: mimetype,
            data: data
        });
...
router.put("/:id", fileUpload, async (req, res) => {
    ...
    try {
        ...
        const result = await Mellis.update(updateData, {
            where: { id },
        });
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        const result = await Mellis.destroy({
            where: { id },
        });
