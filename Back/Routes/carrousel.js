...
router.get("/", async (req, res) => {
    try {
        ...
        let images;
        if (search) {
            images = await Carrousel.findAll({
                where: {
                    name: {
                        [Op.like]: `%${search}%`
                    }
...
router.get("/:id", async (req, res) => {
    ...
    try {
        const image = await Carrousel.findByPk(id);
        if (!image) {
...
router.post("/", fileUpload, async (req, res) => {
    try {
        ...
        const newImage = await Carrousel.create({
            name: originalname,
            type: mimetype,
            data: data
        });
...
router.put("/:id", fileUpload, async (req, res) => {
    ...
    try {
        ...
        const result = await Carrousel.update(updateData, {
            where: { id },
        });
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        const result = await Carrousel.destroy({
            where: { id },
        });
