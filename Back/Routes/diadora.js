...
import fs from "fs";
import Diadora from "../models/diadora.js"; 
...
router.get("/", async (req, res) => {
    try {
        const imgs = await Diadora.findAll()
        res.json(imgs)
...
router.get("/:id", async (req, res) => {
    ...
    try {
        const image = await Diadora.findByPk(id);
        if (!image) {
...
router.post("/", fileUpload, async (req, res) => {
    try {
        ...
        const newImage = await Diadora.create({
            name: originalname,
            type: mimetype,
            data: data
        });
...
router.put("/:id", fileUpload, async (req, res) => {
    ...
    try {
        ...
        const result = await Diadora.update(updateData, {
            where: { id },
        });
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        const result = await Diadora.destroy({
            where: { id },
        });
