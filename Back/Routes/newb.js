...
import fs from "fs";
import Newb from "../models/newb.js"; 
...
router.get("/:id", async (req, res) => {
    ...
    try {
        const image = await Newb.findByPk(id);
        if (!image) {
...
router.post("/", fileUpload, async (req, res) => {
    try {
        ...
        const newImage = await Newb.create({
            name: originalname,
            type: mimetype,
            data: data
        });
...
router.put("/:id", fileUpload, async (req, res) => {
    ...
    try {
        ...
        const result = await Newb.update(updateData, {
            where: { id },
        });
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        const result = await Newb.destroy({
            where: { id },
        });
