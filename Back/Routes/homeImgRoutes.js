...
import ImageHome from "../models/homeImg1.js";
...
router.get("/", async (req, res) => {
    try {
        const imgs = await ImageHome.findAll()
        res.json(imgs)
...
router.get("/", async (req, res) => {
    try {
        ...
        let images;
        if (search) {
            images = await ImageHome.findAll({
                where: {
                    name: {
                        [Op.like]: `%${search}%`
                    }
...
router.get("/:id", async (req, res) => {
    ...
    try {
        const image = await ImageHome.findByPk(id);
        if (!image) {
...
router.post("/", fileUpload, async (req, res) => {
    try {
        ...
        const newImage = await ImageHome.create({
            name: originalname,
            type: mimetype,
            data: data
        });
...
router.put("/:id", fileUpload, async (req, res) => {
    ...
    try {
        ...
        const result = await ImageHome.update(updateData, {
            where: { id },
        });
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        const result = await ImageHome.destroy({
            where: { id },
        });
