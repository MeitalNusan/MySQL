...
import fs from "fs";
import Olympikus from "../models/olympikus.js"; 
...
router.get("/", async (req, res) => {
    try {
        const imgs = await Olympikus.findAll()
        res.json(imgs)
...
router.get("/", async (req, res) => {
    try {
        ...
        let images;
        if (search) {
            images = await Olympikus.findAll({
                where: {
                    name: {
                        [Op.like]: `%${search}%`
                    }
...
router.get("/:id", async (req, res) => {
    ...
    try {
        const image = await Olympikus.findByPk(id);
        if (!image) {
...
router.post("/", fileUpload, async (req, res) => {
    try {
        ...
        const newImage = await Olympikus.create({
            name: originalname,
            type: mimetype,
            data: data
        });
...
router.put("/:id", fileUpload, async (req, res) => {
    ...
    try {
        ...
        const result = await Olympikus.update(updateData, {
            where: { id },
        });
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        const result = await Olympikus.destroy({
            where: { id },
        });
