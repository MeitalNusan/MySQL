...
import fs from "fs";
import Athix from "../models/athix.js"; 
...
router.get("/", async (req, res) => {
    try {
        const imgs = await Athix.findAll()
        res.json(imgs)
...
router.get("/:id", async (req, res) => {
    ...
    try {
        const image = await Athix.findByPk(id);
        if (!image) {
...
router.post("/", fileUpload, async (req, res) => {
    try {
        ...
        const newImage = await Athix.create({
            name: originalname,
            type: mimetype,
            data: data
        });
...
router.put("/:id", fileUpload, async (req, res) => {
    ...
    try {
        ...
        const result = await Athix.update(updateData, {
            where: { id },
        });
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        const result = await Athix.destroy({
            where: { id },
        });
